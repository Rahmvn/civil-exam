-- Removing a question from an editable set must preserve historical attempt reviews.
...
begin
  ...
  if preserve_history then
    ...
    -- The status trigger may archive an otherwise empty set. Keep the
    -- administrator's editable set in its original lifecycle state.
