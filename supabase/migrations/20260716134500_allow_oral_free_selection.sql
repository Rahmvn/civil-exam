-- Oral set 1 should be selectable as the candidate's free module when no free
-- module has been chosen yet. Purchase availability remains separate: paid
-- access is still required for later oral sets and for repeat practice after
-- the free oral set is completed.
...
as $$
  ...
  set_facts as (
    select
      ...
      not exists (select 1 from selected_free) as no_free_module_selected,
  ...
  select
    ...
    case
      ...
      when r.completed_count > 0 and not r.paid_access then false
      when r.set_number = 1 and (r.free_access or r.no_free_module_selected) then true
      else false
    end,
    case
      ...
      when r.set_number = 1 and (r.free_access or r.no_free_module_selected) then 'available'
    ...
    case
      ...
      when not r.paid_access and r.no_free_module_selected and r.set_number = 1 then 'free_batch_available'
    ...
    case
      ...
      when r.no_free_module_selected and r.set_number = 1 then 'Choose this module for your free oral practice.'
