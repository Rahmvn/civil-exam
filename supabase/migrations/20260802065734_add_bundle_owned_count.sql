...
begin
  ...
  select
    ...
    case e.reason
      ...
      when 'not_enough_modules_left' then 'Not enough modules left for this bundle.'
      when 'all_modules_unlocked' then 'You have unlocked all modules in this bundle.'
      when 'full_bundle_new_candidates_only' then 'Available only before buying modules.'
      when 'no_current_saving' then 'No saving on this bundle right now.'
      when 'no_modules_available' then 'No modules are available for this bundle.'
      else 'This bundle is not available for your account.'
