-- An abandoned oral attempt is an explicit exit, not learning progress.
-- Keep it for audit/history, but exclude it from candidate-facing attempt
-- counts and every completion/pass decision.
...
as $$
  ...
  attempt_facts as (
    select
      ...
      sf.practice_set_id,
      count(oa.id) filter (
        where oa.status in ('active', 'completed')
